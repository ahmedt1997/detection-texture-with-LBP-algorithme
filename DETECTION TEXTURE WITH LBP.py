...
textures=['cushion', 'canvas', 'stone','seat','sand','linseeds']

tab_images=[]
tab_hist=[]
for texture in textures:
	print(">>>", "{}.png".format(texture))
	image=cv2.imread("{}.png".format(texture), 0)
	if image is None:
		quit("Probleme image...")
	lbp=feature.local_binary_pattern(image, numPoints, radius, method=method_lbp[id_method_lbp])
	hist_ref, _=np.histogram(lbp, density=True, bins=2**numPoints, range=(0, 2**numPoints))
	
	tab_images.append(image)
	tab_hist.append(hist_ref)

for fichier in glob.glob("textures/*.png"):
	...
	for id in range(len(tab_hist)):
		...
		hist, _=np.histogram(lbp, density=True, bins=2**numPoints, range=(0, 2**numPoints))
